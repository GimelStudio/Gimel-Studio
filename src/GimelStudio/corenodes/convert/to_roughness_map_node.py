...
class NodeDefinition(NodeBase):
    ...
    @property
    def NodeIDName(self):
        return "gimelstudiocorenode_toroughnessmap"

    @property
    def NodeLabel(self):
        return "To Roughness Map"
    ...
    def NodeDescription(self):
        return "Converts the image into a roughness map texture for use in 3D." 
    ...
    def ComputeRoughnessMap(self, image, saturation, brightness, gamma):
        """ Calculates and returns a roughness map. """
        gray_scale_img = cv2.bitwise_not(image)
        roughness_map = cv2.convertScaleAbs(
            gray_scale_img, 
            alpha=saturation, 
            beta=brightness
            )
        gc_roughness_map = self.GammaCorrection(roughness_map, gamma)
        return cv2.bitwise_not(gc_roughness_map)
