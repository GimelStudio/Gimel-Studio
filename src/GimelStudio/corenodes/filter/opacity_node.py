...
class NodeDefinition(NodeBase):
    ...
    def NodePropertiesUI(self, node, parent, sizer):
        current_opacity_value = self.NodeGetPropValue('Opacity')

        opacity_label = wx.StaticText(parent, label="Image opacity:")
        sizer.Add(opacity_label, border=5)

        self.opacity_slider = wx.Slider(
            parent, 100, 25, 1, 100,
            style=wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS
            )
        self.opacity_slider.SetTickFreq(5)
        self.opacity_slider.SetRange(1, 100)
        self.opacity_slider.SetValue(current_opacity_value)
        sizer.Add(self.opacity_slider, flag=wx.EXPAND|wx.ALL, border=5)

        parent.Bind(wx.EVT_SCROLL_THUMBRELEASE, self.OnOpacityChange, self.opacity_slider)

 
    def OnOpacityChange(self, evt):
        self.NodePropertiesUpdate('Opacity', self.opacity_slider.GetValue())
